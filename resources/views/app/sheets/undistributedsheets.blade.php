                                    ...
                                    <div class="panel ">
                                        <div class="panel-heading">
                                            <h3 class="panel-title">
                                                ...
                                                Liste des fiches non attribué
                                            </h3> 
                                        </div>
                                        <div class="panel-body"> 
                                          <div class="row"  style="border: 2px solid #eee;border-radius: 5px;  padding-top: 16px;margin-left: 0px;    margin-right: 0px;">                    
                                          <div class="col-md-9">                    
                                          {!! Form::open(['method' => 'POST','class'=>'form-horizontal','enctype'=>'multipart/form-data', 'route' => 'getUnDistributedSheetsFiltePage']) !!}    
                                          <input name="_token" type="hidden" id="_token" value="{{ csrf_token() }}" /> 
                                            ...
                                            <div class="table-responsive">
                                                <table class="table table-striped table-bordered table-hover" id="undistributedSheetsList">
                                                    <thead>
                                                        <tr>
                                                            <th   hidden >id</th>  
                                                            <th  >N°</th> 
...
$( document ).ready(function() { 
    ...
    if(comingCreatedAt != ''){
        $('#undistributedSheetsList').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('getUnDistributedSheetsFilterData', $created_at) }}',
            columns: [
        ...
        $('#undistributedSheetsList').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('getUnDistributedSheetsData') }}',
            columns: [
                {data: 'id', name: 'id', visible:false }, 
                {data: 'order', name: 'order'}, 
                {               
    ...
    var table = $('#undistributedSheetsList').DataTable();    
  ...
  $('#undistributedSheetsList').on('click', '#openDistributeSheet', function(){ 
  ...
  $('#undistributedSheetsList').on('click', '#openDeletesheet', function(){
    $('#md-delete-sheet-id').text($(this).attr('name')); 
    $('#md_delete_sheet_id').val($(this).attr('value'));
    $('#deleteSheetModal').modal('show');
  });

  $('#undistributedSheetsList').on('click', '#openSendEmailToClient', function(){
